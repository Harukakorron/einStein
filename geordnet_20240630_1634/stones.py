...
class stone:
    ...
    def to_db(self):
        ...
        sql = f"INSERT INTO Steine (Name, Formeleinheit, Verwendung, Härte, Farbe, Zugehörigkeit, Gefahr, Nummer, Oberklasse, Bild) VALUES ('{self.Name}', '{self.Formeleinheit}', '{self.Verwendung}', '{self.Härte}', '{self.Farbe}', '{self.Zugehörigkeit}', '{self.Gefahr}', '{self.Nummer}', '{self.Oberklasse}', '{self.Bild}')"
        cursor.execute(sql)
        connection.commit()
        connection.close()
